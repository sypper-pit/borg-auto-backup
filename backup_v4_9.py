#!/usr/bin/env python3
# backup.py v4.9 - Clean, stable, Python 3.8+ (with --tag support)
...
def do_backup(env, all_mode, tag=None):
    ...
    base_name = (
        ...
        f"{time.strftime('%Y-%b-%d_%H-%M-%S')}"
    )
    if tag:
        archive = f"{base_name}-{tag}"
    else:
        archive = base_name
...
def main():
    ...
    if args.list:
        ...
        do_backup(env, args.all, tag=args.tag)  # ✅ исправлено
    elif args.restore:
